...
export function filtrarPergunta(p: Pergunta, rows: Respondente[], f: FiltroPergunta): Pergunta {
  const sel = rows.filter(
    (r) =>
      (!f.sexo || r.sexo === f.sexo) &&
      (!f.idade || r.idade === f.idade) &&
  ...
  const items: ResultItem[] = labels.map((label) => ({
    label,
    n: counts.get(label)!,
  ...
  if (p.tracking) {
    ...
    tracking = {
      ...
      points: p.tracking.points.map((pt) => {
        ...
        if (doDia.length) {
          for (const s of series) ponto[s] = pct1(doDia.filter((r) => r[p.id] === s).length, doDia.length)
        }
        return ponto
      }),
    }
